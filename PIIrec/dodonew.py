...
dataFileName=dir+"dodonew_cleaned.txt"
# dataFileName="\clean_data.csv"
output=dir+"Recognize_data.csv"
pcfg_password_file=dir+'dodonew_pcfg_format_password.csv'
replaceFileName=dir+"Replace_word.csv"
irreplaceFileName=dir+"dodonew_unmatch-v2.0.csv"
# data=read_file(dataFileName)
# _,_,d=data.value
size=0
count=0
PII_usage={'username':0,'email':0,'id':0,'birth':0,'name':0,'phone':0,'sex':0,'postcode':0}
...
def stat_reference(format_passwords):
    ...
    for password in format_passwords:
        ...
        trans={'U':'username','E':'email','B':'birth','N':'name','P':'phone','I':'id'}
        for ref in ref_list:
            if ref in trans:
...
if __name__=="__main__":
    ...
    for i,PII in enumerate(PIIs):
        ...
        if len(ID) == 18:
            ...
            C = ID[-4:]
            # print("birth: %s/%s/%s" % (Y, M, D))
            idPattern = ['%s%s%s' % (A, B, C),'%s%s' % (A, B),'%s'%(A),'%s'%(B),'%s%s'%(B,A),'%s'%(C),]
            idPattern.sort(key=lambda i: len(i), reverse=True)
            # print(birthPattern)
            for pattern in idPattern:
                if pattern in passwords[i]:
                    reg = re.compile(re.escape(pattern), re.IGNORECASE)
                    temp = str(passwords[i])
                    for it in reg.finditer(origin_passwords[i]):
                        ...
                        length = len(it.group())
                        for index in range(start_index, start_index + length):
                            format_password[index] = 'I'
                        temp = reg.sub('$', str(temp))
                    raw_password.append(passwords[i])
                    replace_PII.append(['id', pattern])
                    passwords[i] = temp
        #Name
        acronym=""
        for name_indice in name:
            if name_indice!='':
        ...
        password_substring=[[origin_passwords[i][begin:begin + x + 1],begin] for x in range(3,len(temp)) for begin in range(len(temp) - x)]
        ...
        for substring in password_substring:
            index=['']
            typelist=['U','E','N','I','P','B']
            typelist2=['username','email','name','id','phone','birth']
            for j in [0,4]:
